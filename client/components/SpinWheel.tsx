...
export default function SpinWheel({ rewards = [], onResult, size = 240 }: { rewards?: Reward[]; onResult?: (r: Reward)=>void; size?: number }){
  ...
  function spin(){
    ...
    if(wheelRef.current){
      wheelRef.current.style.transition = 'transform 5s cubic-bezier(.2,.7,.2,1)';
      wheelRef.current.style.transform = `rotate(${finalDeg}deg)`;
    }
    setTimeout(()=>{
      setSpinning(false);
      const r = rewards[winnerIndex] || {id:String(winnerIndex), label:`Prize ${winnerIndex+1}`};
      onResult?.(r);
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="relative" style={{width: size, height: size}}>
        <div ref={wheelRef} className="rounded-full overflow-hidden" style={{width: size, height: size}}>
          <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} xmlns="http://www.w3.org/2000/svg">
            <g transform={`translate(${size/2},${size/2})`}>
              {Array.from({length: segments}).map((_,i)=>{
                ...
                const color = rewards[i]?.color || (i%2? '#F4A261':'#E27D60');
                return <path key={i} d={d} fill={color} stroke="#145956" strokeWidth={2}/>;
              })}
            </g>
          </svg>
        </div>
      ...
      <button disabled={spinning} onClick={spin} className={`pixel-button mt-3 ${spinning? 'opacity-60 cursor-not-allowed':''}`}>Spin</button>
    </div>
  );
}
